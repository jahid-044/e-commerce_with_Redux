...
export default function Nav() {
  const { cart, modalCall, setModalCall, totalQuantity } = useContext(modalContext);
  function getTotalQuantity() {
    let total = 0;
    for (const itemKey of cart) {
      total += itemKey.quantity;
    }
    return total;
  }
  return (
                ...
                <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                  ...
                  <li className="font-sans block mt-4 lg:inline-block lg:mt-0 lg:ml-6 align-middle text-red-200 hover:text-gray-300">
                    <a role="button" onClick={() => setModalCall(!modalCall)} className="relative flex">
                      ...
                      {
                        getTotalQuantity() ? (<span className="absolute right-0 top-0 rounded-full bg-red-600 w-4 h-4 top right p-0 m-0 text-white font-mono text-sm  leading-tight text-center">{getTotalQuantity()}
                        </span>) : null
                      }
